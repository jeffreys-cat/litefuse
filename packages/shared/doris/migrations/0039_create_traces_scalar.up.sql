...
-- is_root = 1). Serves the trace-list scalar queries (traces.all / countAll):
-- every column the list returns is a root-pick scalar, so the list can read this
-- table flat — no GROUP BY, no traces_mv aggregate, no MV-staleness compensation.
--
-- Deliberately NOT here:
--   * input/output (even trimmed) — the list doesn't render them; keeping the
--     giant payloads out keeps the table narrow. Content search stays on
--     events_full's full-text indexes.
