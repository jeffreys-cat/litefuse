...
export const eventsTableNativeUiColumnDefinitionsForDoris: UiColumnMappings = [
  ...
  },
  {
    ...
    tableName: "events_proto",
    select:
      "if(isNull(e.completion_start_time), NULL, milliseconds_diff(e.completion_start_time, e.start_time) / 1000)",
    typeOverwrite: "Decimal64(3)",
  },
  {
    uiTableName: "Latency (s)",
    ...
    tableName: "events_proto",
    select:
      "if(isNull(e.end_time), NULL, milliseconds_diff(e.end_time, e.start_time) / 1000)",
    typeOverwrite: "Decimal64(3)",
  },
  {
    uiTableName: "Tokens per second",
    ...
    tableName: "events_proto",
    select:
      "(sum(if(MAP_CONTAINS_KEY(usage_details,'output'),usage_details['output'],0))) / (milliseconds_diff(any_value(e.end_time),any_value(e.start_time)) / 1000)",
  },
  {
    uiTableName: "Input Cost ($)",
    uiTableId: "inputCost",
