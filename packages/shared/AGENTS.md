...
- Optional pg-boss cron schedule definitions and helpers: `src/server/pgboss/*`
...
1. Update optional schedule definitions in `src/server/pgboss/pgBoss.ts`.
2. Keep existing `src/server/redis/*` BullMQ queue helpers intact unless the
   queue contract itself changes.
3. Update worker registration/gating so pg-boss and BullMQ repeatable
   scheduling do not run the same trigger at the same time.
4. Add or update worker tests for cron pattern, payload, and env gating.
