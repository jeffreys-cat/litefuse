...
const mockedGetInvertedIndexColumns =
  getInvertedIndexColumns as jest.MockedFunction<
    typeof getInvertedIndexColumns
  >;
...
describe("Lucene case fixtures", () => {
  if (luceneCases.length === 0) {
    it("has at least one lucene case fixture defined", () => {
      throw new Error(
        "No lucene case fixtures found under src/services/__tests__/cases",
      );
    });
    return;
  }
  ...
  it.each(
    ...
    const metadataLookup = Object.entries(columns).reduce<
      Record<string, ColumnFixture>
    >((acc, [key, value]) => {
      acc[key] = value;
    ...
    mockedGetColumn.mockImplementation(async ({ column }) => {
      const lookupKey =
        column in metadataLookup ? column : column.toLowerCase();
      return metadataLookup[lookupKey] ?? null;
    });

    mockedGetInvertedIndexColumns.mockImplementation(
      async () => invertedIndexColumns,
    );
    ...
    if (!expectedWhere && !expectedSql) {
      throw new Error(
        `Fixture "${testCase.fileName}" must specify expectedWhere or expectedSql`,
      );
    }
