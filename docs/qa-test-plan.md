...
> 目标：验证 [langfuse-doris](https://github.com/selectdb/langfuse-doris)（Doris 后端，`master` 分支）与 langfuse 原版（ClickHouse 后端，[langfuse/langfuse](https://github.com/langfuse/langfuse) tag `v3.16.0` 或 `v3.16.1`）在数据写入、接口返回、Web 功能三个层面的一致性。
...
- langfuse-ck 版本：v3.16.0 / v3.16.1（https://github.com/langfuse/langfuse）
- Doris 版本：xxx
- ClickHouse 版本：xxx
